...
def build_quantization_table_fast(bit_depth):
...
def quantize_channel_fast(channel_array, q_table):
...
def quantize_color_image_fast(img_array, bit_depth):
    ...
    r_quantized = quantize_channel_fast(img_array[:, :, 0], q_table)
    g_quantized = quantize_channel_fast(img_array[:, :, 1], q_table)
    b_quantized = quantize_channel_fast(img_array[:, :, 2], q_table)
    
    return (r_quantized, g_quantized, b_quantized), q_table

def reconstruct_color_image_fast(quantized_channels, q_table, original_shape):
